...
class Provider(BaseProvider):
    ...
    classroom_types = (
        ...
        'Silent Study Kişisel Erişim Merkezi', 'Video-Audio G Kişisel Erişim Merkezi',
        'Self Access Center',
        'Simülasyon Laboratuarı', 'Tasarım Stüdyoları', 'Tekstil Arşivi', 'Tekstil Baskı Atölyesi'
    )

    classroom_seating_plans = ('Theatre', 'Round Table',)

    classroom_formats = ('{{classroom_code}} - {{classroom_type}}',
                         '{{classroom_code}} - {{classroom_seating_plan}}')

    @classmethod
    def classroom_seating_plan(cls):
        return cls.random_element(cls.classroom_seating_plans)
